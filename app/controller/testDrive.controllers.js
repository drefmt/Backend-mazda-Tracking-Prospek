...
exports.createTestDrive = async (req, res) => {
  try {
    ...
    if (existingTestDrive) {
      logger.warn(`Test drive sudah ada untuk prospekId: ${prospekId} oleh user: ${userId}`);
      return res.status(409).send({ message: "Test drive sudah ada" });
    }
...
exports.findAllTestDrive = async (req, res) => {
  try {
    ...
    if(userLevel  === 'svp') {
      ...
      .populate("prospekId", "name whatsappNum").sort({createdAt: -1});
    } else {
    ...
    logger.info(`Data Test Drive diambil oleh user: ${userId}`, { count: testDrives.length });
...
exports.findTestDriveById = async (req, res) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
    const testDrive = await TestDrive.findOne({ _id: id, salesId: userId })
      .populate("prospekId", "name whatsappNum");

    if (!testDrive) {
      logger.warn(`Data test drive tidak ditemukan untuk ID: ${id} oleh user: ${userId}`);
      return res.status(404).send({ message: "Data test drive tidak ditemukan" });
    }
    logger.info(`Data test drive ditemukan untuk ID: ${id} oleh user: ${userId}`);
...
exports.updateTestDrive = async (req, res) => {
  try {
    ...
    if (!updateTestDrive) {
      logger.warn(`Test drive tidak ditemukan untuk update ID: ${id} oleh user: ${userId}`);
      return res.status(404).send({ message: "Test drive tidak ditemukan" });
    }

    logger.info(`Test Drive berhasil diperbarui untuk ID: ${id} oleh user: ${userId}`, { updateTestDrive });
    res.status(200).send({
...
exports.deleteTestDrive = async (req, res) => {
  try {
    ...
    const deleteTestDrive = await TestDrive.findOneAndDelete({ _id: id, salesId: userId });

    if (!deleteTestDrive) {
      logger.warn(`Data Test Drive tidak ditemukan untuk dihapus ID: ${id} oleh user: ${userId}`);
      return res.status(404).send({ message: 'Data Test Drive tidak ditemukan' });
    }

    logger.info(`Test drive berhasil dihapus untuk ID: ${id} oleh user: ${userId}`);
    res.status(200).send({ message: "Test drive deleted successfully" });
  } catch (error) {
